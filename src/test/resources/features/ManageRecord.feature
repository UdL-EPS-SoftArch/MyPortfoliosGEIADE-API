Feature: Manage Project
  ...
  Scenario: Create a project with PRIVATE visibility
    Given There is a registered user with username "user" and password "password" and email "user@sample.app"
    And I login as "user" with password "password"
    And I prepare a project with name "Secret Project" and description "Internal" and visibility "PRIVATE"
    When I send the create project request
    Then The response code is 201
    And The project visibility should be "PRIVATE"
  ...
  Scenario: Retrieve list of all projects
    Given There is a registered user with username "user" and password "password" and email "user@sample.app"
    And I login as "user" with password "password"
    And There is a project with name "Project A" and description "desc" and visibility "PUBLIC"
    And There is a project with name "Project B" and description "desc" and visibility "PUBLIC"
  ...
  Scenario: Update a project name
    ...
    And The project modification date should be set

  Scenario: Update project visibility to PRIVATE
    Given There is a registered user with username "user" and password "password" and email "user@sample.app"
    And I login as "user" with password "password"
    And There is a project with name "Visibility Test" and description "desc" and visibility "PUBLIC"
    When I update the project named "Visibility Test" setting visibility to "PRIVATE"
    Then The response code is 200
    And The project visibility should be "PRIVATE"
  ...
  Scenario: Delete a non-existing project
    Given There is a registered user with username "user" and password "password" and email "user@sample.app"
    And I login as "user" with password "password"
    When I delete the project with id 99999
    Then The response code is 404
